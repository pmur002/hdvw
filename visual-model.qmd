...
back to the original information---a **decoding**.

We describe the transformation of data values into data symbols
as **encoding** the data values and
later chapters will explore many different encodings.
In this chapter, we focus on the reverse operation:
...
each box, click on each of the coloured circles, which will produce a colour
wheel.  Click (in the same order) on the colour wheel where it matches the
colour of the original circles.  This will present the results as small black
...
#| layout-ncol: 2
#| fig.height: 6
#| fig-cap: We have to work much harder to identify a unique item when there is more going on.
#| fig-subcap:
...
the visual system automatically decodes groups of items 
from an image.
...
::: {.content-block style="font-style: italic;"}
