...
In @sec-intro, we established that a data visualisation can be thought
of as an **encoding** from data values to data symbols and
extracting information from a data visualisation involves 
**decoding** from data symbols back to data values.

We will consider many different ways to encode information in this book.
For example, we have already seen that we
can encode data values as different-coloured lines 
to create a line plot (@fig-crime)
or we can encode data values as different-coloured rectangles to
create a heatmap (@fig-crime-heatmap).
However, one thing that all of encodings that we consider
...
involves the human visual system.
A good encoding will produce a data visualisation that
the visual system is good at decoding.
In this chapter, we look at some basic properties
of the visual system.[^simple-visual-models]
If we understand how the visual system works, we will begin
to understand how data visualisations work.
...
For example,
@fig-visual-model-demo shows an image that is made up of over
17,000 individual visual elements, but we can easily consume
the image and identify features without any difficulty.
...
simple information.
We will look more at the encoding of data values as visual features
in @sec-encoding-visual-features.

In addition, a smaller amount of 
more complex information will be decoded from
higher-level processing of low-level visual features.
We will look more at the decoding of multiple visual features in
@sec-summaries.

As an example, the image in 
@fig-visual-model-demo encodes
wind data values as many small triangles.[^NOMADS]
Wind direction is encoded as the angle of the triangles
and wind strength is encoded as the
length of the triangles. 
Colour is used to encode whether the location is over the sea (blue)
or over the land (green).
...
The visual system is able to 
effortlessly decode the basic **visual features**---angle, 
size, and colour---of many triangles at once.
...
Importantly, many pieces of information, such as lighter and darker regions
and even the shape of New Zealand, are decoded automatically.
We do not have to deliberately search for these features
and we do not have to be prompted by a question of interest to look
for these features.
...
We will look more at the use of visual shapes and visual objects
in a data visualisation in 
@sec-visual-shapes and @sec-visual-objects.
...
and see everything in the image. 
Instead, we perform a series of **fixations**, which are
...
The limits on visual memory reflect well-known limits on working memory---we
can only memorise a small number of visual features at once.
On the other hand,
those limits can also be worked around via familiar mechanisms such
as chunking.[^miller]
...
As an example, the image in @fig-visual-model-demo-dots
is based on the same wind data as @fig-visual-model-demo.
However, in @fig-visual-model-demo-dots the data symbol
is a circle rather than a triangle, with the wind strength
encoded as the size of the circle and the wind direction 
encoded as the colour of the circle: 
southerly winds are black, westerly winds are red, 
easterly winds are green, and northerly winds are blue.
This data visualisation is problematic for several reasons,
several of which relate to the demands on visual focus
and visual memory.

::: {#fig-visual-model-demo-dots}
::: {.content-visible when-format=pdf}
...
As with @fig-visual-model-demo,
we automatically identify some features of 
@fig-visual-model-demo-dots
automatically.  For example, we can immediately identify different regions
of different colours, plus darker and lighter regions.
However, if we want to decode wind information from 
@fig-visual-model-demo-dots
we have more work to do.

In @fig-visual-model-demo, we can naturally decode wind 
direction from the angles of the triangles, but
the decoding of wind direction in 
@fig-visual-model-demo-dots requires a legend.
For example, there needs to be an explanation that black dots
represent southerly winds.
There is no natural correspondence between the colours used for
the encoding and the wind directions.
This means that we have to switch focus between the dots in the main
image and the dots in the legend.
Furthermore, we may have to switch focus repeatedly if 
we forget one of the encodings.
Without looking at @fig-visual-model-demo-dots, can you recite
the colour-to-wind-direction encodings yet?
This requirement to switch focus and memorise encodings makes it
more difficult to decode the wind information from the image in 
@fig-visual-model-demo compared to the image in @fig-visual-model-demo.

By contrast, in @fig-visual-model-demo, there was no need for a legend because
we were able to decode wind direction naturally
from the angles of the triangles.
We will look more at this sort of natural decoding of information
in @sec-congruence.
...
processes decode some information from an image without any mental effort.
...
@fig-visual-model-demo
that encodes the location of the strongest wind.
This triangle is very easy to identify, even amongst
...
As an example, it is not only easy to identify the single red triangle in 
@fig-visual-model-demo,
it is actually quite difficult to ignore it.
...
As an example,
we automatically identify separate groups of triangles in
@fig-visual-model-demo (land versus sea)
and separate groups of dots in 
@fig-visual-model-demo-dots (different wind directions),
just based on the common colours of the triangles or dots.
...
As an example, the image in 
@fig-visual-model-demo-barb
encodes a subset of the wind data from
@fig-visual-model-demo and @fig-visual-model-demo-dots.
The data symbol in 
@fig-visual-model-demo-barb
is a *wind barb*, which represents the wind direction as the angle
of a line and the wind strength as small tick marks on the tail
of the line.
...
There is also an outline of the coast of (the North Island of) New Zealand
in @fig-visual-model-demo-barb. 
Although the outline of the coast overlaps with the wind barbs in many
places, in most places
we are able to decode the coast as a single continuous shape
separate from the individual barbs.
The continuous coastline crossed by smaller barbs is a simpler decoding
than any other interpretation.

## Visual tasks {#sec-tasks}

Identifying whether visual elements are the same or different
(@sec-differences and @sec-visual-model-groups) 
...
An example of a relatively simple visual task is the decoding
of wind direction from 
@fig-visual-model-demo.
For example, at the right of
@fig-visual-model-demo,
about half way up the image, 
we can see a region where the wind is circling around a point.
By contrast, identifying that location in 
@fig-visual-model-demo-dots
is much more difficult.
The location in
@fig-visual-model-demo-dots
is marked by a point where all four colours
meet, but decoding that the wind is circling at that
location requires decoding the four wind directions and
then combining the four directions to conclude that the
wind directions form a circle.
...
against us in @sec-colour-relative.
