...
In this book, we are going to explore a range of ways that
...
The choice of encoding depends on how well information can be
**decoded* from the data visualisation.
...
we need to know how well information can be decoded from 
...
process many small **visual features** at once.
...
On the other hand, if we directly **encode** information as **visual shapes**
or **visual objects**, then it will be possible to decode a smaller 
amount of information, but that information may be more complex.

For example, @fig-visual-model-demo-2 shows an image that is made
up of regions of different colours and a grey map of New Zealand.
This is based on the same data as @fig-visual-model and
the colour of the regions represent different ranges of 
wind strength (darker is stronger).
The
grey map **visual object** decodes very easily to the country of New Zealand 
(especially for Kiwis) and the **visual shapes**
of the coloured regions decode to areas of
similar wind strength.

::: {#fig-visual-model-demo-2}
::: {.content-visible when-format=pdf}
\includegraphics[width=.5\textwidth]{Data/Weather/NOMADS-high.png}
:::

::: {.content-visible when-format=html}
![](Data/Weather/NOMADS-high.png)
:::
...
This is effectively an argument for keeping things simple.
...
#| echo: false
#| label: fig-symmetry
#| fig-cap: We automatically perceive groups from symmetric arrangements.  The pair of symmetric curves on the left naturally go together, but the pairs of curves in the middle and to the right do not.
#| fig.height: 2
library(vwline)
...
This task in requires much more deliberate effort 
