...
In particular, in order to understand the effectiveness of a mapping
from information to a visual representation,
we need to know how we get from visual representations
back to the original information.
In @sec-intro, we described a mapping as an
**encoding** of information as a visual representation.
In this chapter, we will consider the reverse operation:
the **decoding** of a visual representation back to the original information
(@fig-decode).
...
different mappings that we consider throughout the book.[^simple-visual-models]
...
In order for us to perceive a data visualisation, light must emit from 
...
> One reason why data visualisation is effective is
  because the visual system gathers
...
**position**, **angle**, and **colour**.[^feature-maps]
...
[^feature-maps]:
    ...
    within the field of view.[@ware2021information, page=146]

[^ware-model]:
    A slightly more detailed overview is provided in
    @ware2021information (page 20).
...
**angles**, and **colours**.
The angles of the triangles represents wind direction and the 
...
At a higher level of processing, darker and lighter **shapes** are
...
The small lines in @fig-visual-model-demo
convey a lot of simple pieces of information---what
is the wind strength and direction at each location---though
we have to focus on a particular spot in order to extract 
a single piece of information.
The shapes of darker and lighter regions convey several more complex ideas like
areas of stronger and lighter winds.
...
> Encoding data values as simple visual features is effective
  at conveying large amounts of simple information
  because the visual system decodes simple visual features rapidly
  and all at once.

> Encoding data values as more complex visual shapes or visual objects
  is effective because more complex information 
  can be decoded from
...
[^extinction]:
    The extinction illusion is related to the Hermann grid illusion
    [@doi:10.1068/p2985].
...
original image and also text at the top and bottom of the image.
...
> A data visualisation may fail if it 
  requires the viewer
...
we are likely to need to remember information from a previous area of focus.
...
[^memory-limits]:
    ...
    "A ... severely limited capacity is the restricted number of stimuli that
    can be held in VSTM [visual short-term memory]" 
    [@MAROIS2005296]
...
::: {#fig-memory .content-visible when-format=pdf}
\afterpage{
\begin{figure}[t]
...
visual features such as the colours of circles (@fig-memory),
...
In other words, can remember several **visual shapes** or 
...
For example, can hold in memory multiple country flags even though
...
> A data visualisation will be effective if important differences
  in the data are encoded using different colours or sizes,
...
> A data visualisation will be effective if it uses basic visual
  features like colour to draw the viewer's attention to
  important features.
...
> A data visualisation will be effective if it includes instructions
  that direct the reader's attention to important features.
...
(@sec-differences) is that we can easily identify items that are the same.
...
> A data visualisation will be effective if it relies on 
  ...
  conscious mental effort.[^cog-effort]
...
[^cog-effort]:
    ...
    cognitive processes"
    [@carpenter1998graph, page=98]

## Dangers of the visual system
...
from a very simple arrangement of simple shapes.
...
#| echo: false
#| label: fig-illusion
#| fig-cap: The two horizonal lines in the Muller-Lyer illusion on the left have exactly the same physical length, but the top line appears longer.  The two inner circles in the Ebbinghaus illusion on the right have exactly the same physical radius, but they appear to be different sizes.
#| fig.height: 2
grid.newpage()
...
@fig-vert-horiz shows another illusions involving simple lines.
...
[^vert-horiz]:
    An early in-depth exploration of the 
    Vertical-Horizontal Illusion was provided by
    @KUNNAPASTM1955AAOT.
...
These visual illusions demonstrate that the visual system 
can produce incorrect results for 
...
differentiated and grouped based on basic visual features.
...
properties of 
the visual system and it will avoid the undesirable properties
