...
**TODO**
Add note that we have moved beyond what is normally considered data values and data symbols for encoding
...
**TODO**
If use bullseye plot in text case study, repeat correlation matrix
here, but do legend with example circles rather than colour gradient.
...
grid.newpage()
pushViewport(viewport(y=.1, height=.9, width=unit(.9, "snpc"), just="bottom"))
print(gg, newpage=FALSE)
grid.force()
...
pushViewport(viewport(y=0, height=.1/.9, just="top", width=.8))
cols <- hcl.colors(7, "Blue-Red 3")
grid.circle(1:7/8, r=unit(4, "mm"),
            gp=gpar(col="black", fill=cols))
grid.text(round(seq(-1, 1, length.out=7), 2), 1:7/8,
          gp=gpar(col=rep(c("white", "black", "white"), c(2, 3, 2)),
                  fontsize=8))
popViewport(2)
```
