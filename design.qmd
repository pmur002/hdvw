...
# Composing Encodings {#sec-design}
...
organisation of a data visualisation.
...
Another way to say this is that the we decode the 
...
data visualisation.  Similarly, the horizontal position of the x-axis title
...
differences.
Put simply, the differences make the data visualisation more complex.
...
[^cigarette]:
    Cigarette image by [rawpixel.com](https://www.rawpixel.com/image/6482159/png-sticker-public-domain), Public Domain.

    Cigarette ash by [2happy](https://www.stockvault.net/photo/119673/inffast#),
    CC0 licence.
...
smokers <- data.frame(year=factor(c(2006, 2013, 2018, 2023)),
                      ...
                      white=5:2*100000)
notrun <- function() {
smokers$white <- smokers$count - c(5, 20, 40, 60)*10000
cigImg <- readPNG("Images/cig-vert.png")
ashImg <- readPNG("Images/ash.png")
cig <- function(data, coords) {
    clip <- rectGrob(x=coords$x, y=0, just="bottom",
                     height=coords$y)
    grobTree(rasterGrob(cigImg, y=-.2, just="bottom", width=unit(15, "mm")),
             vp=viewport(x=coords$x, 
                         clip=clip))
}
ash <- function(data, coords) {
    clip <- rectGrob(x=coords$x, y=coords$y, just="bottom",
                     height=coords$yend - coords$y)
    grobTree(rasterGrob(ashImg, y=coords$y, just="bottom", 
                        width=unit(15, "mm")),
             vp=viewport(x=coords$x, 
                         clip=clip))
}
ggplot(smokers) +
    grid_group(cig, aes(year, count, yend=white, group=year)) +
    grid_group(ash, aes(year, white, yend=count, group=year)) +
...
@fig-rep-none shows a variation on @fig-rep-guide with a modification to the
...
This is clearly an ineffective data visualisation.
It is possible to decode that there are two groups of data symbols in the
plot, but it is impossible to decode which of those two groups is North and
which is South.[^inconsistent-dissonant]
The point of @fig-rep-none is merely to
highlight the critical importance of maintaining consistent encodings between
different elements of a plot.[^CRAP-rep]

[^inconsistent-dissonant]:
    This *inconsistent* encoding could also be described as a 
    *dissonant* encoding (@sec-dissonance).
...
It is much easier to compare the colour of one of the circular data symbols
...
[^trump-bill]:
    This data visualisation is based on a plot from the 
...
[^data-ink-ratio]:
    This also relates to Tukey's *data-ink ratio*
    [@tufte1983visual].
