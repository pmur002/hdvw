...
a encoding that **encodes** data values as the visual features of a data symbol
(@fig-visual-feature).
For example, a bar plot,
like @fig-bar, encodes the total number of youth offenders to the **lengths**
of bars.
...
(@fig-stat-vis-decode).
For example, the median values are encoded to the horizontal **positions** of 
thick vertical lines and the interquartile ranges are encoded to the
...
number of points scored encodes as the horizontal **position** of
...
that identify unusual items (@sec-differences) and identify groups of items 
...
gg <- ggplot(rwcPoints) +
    ...
    geom_errorbar(aes(year, mean, ymin=lower, ymax=upper)) +
    scale_y_continuous(limits=c(0, NA), expand=expansion(c(0, .05)))
grid.newpage()
pushViewport(viewport(width=.8, height=.8))
print(gg, newpage=FALSE)
