...
[^info-vis-ref-model]:
    ...
    basis for some of the hand-waving that I do in this chapter, particularly
    ...
    Could also refer back to @fig-log.  A "data summary" could include
    any sort of data transformation.  In a log plot, what you can
    decode to is the logged values (although those are not usually
    what is used for labels).

    Wilke points out that log scales are appropriate for ratios(?)
    https://clauswilke.com/dataviz/coordinate-systems-axes.html#nonlinear-axes I
    still think that is difficult for people to decode
...
**TODO**
Shift box plots to visual shapes ?
Start here with histogram instead?
...
[^ensembles]:
    ...
    scatter plots).
    Mention "subitizing" (rapid identification of counts below a
    certain threshold) Ware 4th p 173.
...
**ALSO MENTION** textures here? 
Ware 4th ~p212
Large numbers of data symbols, especially in regular arrangement
effectively create a set of textures that the visual system
can identify and differentiate into regions(?)
Refer back to @fig-visual-model-demo.
May need to add something about texture as low-ish level visual feature
back in visual model?
NOTE that only small number of *distint* textures are possible.
...
**TODO**
footnote acknowledge that bar plots are also actually
...
+ For a footnote ...
  ...
  "In a sense, it might be construed as an insult to a man’s intelligence to
   ...
   object be to get a story across, you are justified in taking that means which
   will encounter least resistance, and in making your story as simple as
   possible. For publicity purposes, the pie-chart is therefore almost
   invariably better than the bar."
  [p98]
