...
broad, deep, and detailed data visualisation literature.  There is a need for a
...
[^need-for-simple]:
    ...
    Sonning (2022) deconstructs a line plot in great detail - we want
    to provide something similar, but which is more accessible to
...
Statistics, Computer Science, Psychology, and Cartography.[^summary]
...
[^bertini]:
    Nice example that shows other things people are concerned about
    from Enrico Bertini
    "How NOT to Lie with Charts"
    https://filwd.substack.com/p/how-not-to-lie-with-charts
    Mention that lots of authors like Cairo and Munzner tackle
    the entire data vis process.
...
And footnote that acknowledges 
...
[^ware-model]:
  cite Ware (4th) p229-330, also fig 6.1
...
[^same-diff]:
  We hang our hat on this several times later on, so it would be good
...
When we visualise qualitative data, we will be able to rely
on the automatic identification of different groups of data
symbols based on visual similarities.

> A data visualisation of qualitative data may be effective if it
  relies on the automatic identification of visual groups
...
items have a symmetric arrangement.
...
they already want to go rather than trying to force the viewer
to perceive a more complex interpretation.

> A data visualisation may be effective if it is orderly and
...
## The dangers of the visual system

Visual illusions.

The fact that vertical lines appear longer than horizontal lines
is a good simple one to demonstrate and relate to data vis
(both L and T versions).
...
are fundamental to a data visualisation.
We have also established some basic properties of
the visual system and we have identified
three levels of visual processing:  visual features, visual shapes,
and visual objects.

There are many 
possible ways to map from data values to data symbols.
For example, in @sec-mappings, we saw that the same data values could be
mapped in different ways to produce different data visualisations
(@fig-crime and @fig-crime-heatmap).
 
On one hand, there are many
possible encodings to choose from and, on the other hand, 
there are many possible data symbols to map to.
...
and **one** data value maps to **one** data symbol
(@fig-visual-feature).  
...
the total number of
crimes for different ethnic groups.  The data values are shown
...
[^visual-features]:
    Mention different terminologies:
    visual feature vs visual channel vs visual dimension.
...
[^matching-data-type]:
  ...
  Pinker 1990, p. 111 talks about the different types of data
  that can be extracted from different visual representations.
...
[^pie-encoding]:
    cite the paper that looked at people decoding from area vs angle
    vs chord(!)  vs arc length(!) for pie charts
...
This section covers some more details about how the **length**
...
common baseline (similar to the accuracy of comparing **positions**;
@sec-unaligned-pos).
...
[^dark-is-more]:
    ...
    Schloss et al (2018)
    Evidence for dark-is-more bias
...
other as possible, we also 
have to take into account that some viewers may have difficulty
perceiving differences between particular colours (@sec-CVD).

Fortunately, researchers have produced a number of 
predefined colour palettes that we can make use of.[^colorspace]
...
There are also many helpful tools online.[^adobe]

[^colorspace]:
    cite R Journal article with Achim
...
a **longer** bar naturally conveys the idea of **more**.[^prop-ink]
...
[^info-vis-ref-model]:
  ...
  on decoding to data summaries rather than raw data.
  Also
...
[^Anscombe]:
    Cite Anscombe's quartet and the dinosaur one.
...
[^redundant]:
  Mention the conflicting opinions on redundant encoding:
  Tufte and others against;  some studies for (including hofmann?)
...
> One reason why a line plot is *not* effective for decoding
...
This section looks at the role that 
...
in a report and 
@sec-exec makes use of colour and angle to create visual connections
...
also common to see axis titles (see @sec-terminology).
...
more usefully, 
data summaries that the data visualisation is designed to show.[^top-down]

[^top-down]:
  Mention idea of top-down as opposed to bottom-up attention.
  With cite.
...
A word cloud of the frequency of words in the Wikipedia page for the Rugby World Cup.[^wordcloud-pkg]
:::

[^wordcloud-pkg]:
    This image was produced using the R package {wordcloud} [@pkg-wordcloud].
...
[^table-guidelines]:
  ...
  Also get a Wainer reference in here?
...
## Mapping organisation {#sec-map-org}
...
## The dangers of inconsistent mappings
...
* need citations for more is more and dark is more and part-to-whole.
  "More of what? Dissociating effects of conceptual and numeric mappings on 
   interpreting colormap data visualizations"
  Soto et al (2023)
  Dark-is-more bias (in colours) vs high-is-more bias (in colour legend)
  on colourmaps (choropleth maps).
