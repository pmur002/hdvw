...
**TODO** MUST move this example to AFTER learned decodings,
perhaps even after text
(because it is a conflict between explicit encoding and LEARNED
decoding NOT between explicit encoding and IMPLICIT decoding).
...
A colour can have multiple learned mappings, e.g., red -> hot and
...
A visual feature may have more than one learned encoding, which means that we
may decode more than one value from the visual feature.
:::

**ALSO** you can really f**k things up if you create something dissonant
with a learned decoding, e.g., use the wrong flag for the wrong
country.  Note how much worse this is than just swapping a couple of
the shapes in @fig-scatter-country
...
For example, the horizontal position of the text by itself
allows us to perceive that some counts are (much) larger than others.
...
In the original Figure @fig-crime, the shapes of the text labels on the legend
...
So the text labels in the legend are the most important data symbols
in @fig-crime that allow us to decode the colours of the lines.
...
are decreasing.
It is only the relative position of the lines to the axis tick labels
(and grid lines)
that allows us to decode from the lines to (approximate) data values.
...
**TODO**
add annotation example => WAY more complex information
