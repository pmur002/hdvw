...
**TODO**
also mention "story-telling" or "narratives" ?
...
**data summaries** to visual features (@fig-stat-vis-decode).
...
The only difference from previous sections
is that we are mapping **data summaries**
...
## Visual summaries {#sec-data-vis-decode}
...
a data point and the hemisphere maps to the vertical
...
However, the data summaries for a histogram are more complicated than
...
We have so far only looked at mappings 
...
of offenders to length is effective because is uses a visual feature that
is appropriate for quantitative data and a visual feature that is accurate 
and the lengths are all aligned.
...
maps to a single data symbols (@fig-two-visual-features).
