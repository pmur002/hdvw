...
#| echo: false
#| label: fig-pie-hue
#| fig-cap: A data visualisation of the number of offenders for different levels of crime severity.  We can decode different levels of crime from the different colours of the wedges.
...
#| fig-cap: A data visualisation of the number of offenders for different levels of crime severity.  We can decode different levels of crime from the different colours of the wedges *and* we can decode an ordering of the levels of crime from the colours of the wedges.
ggplot(crimeLevel2021) +
...
## Decoding colour is relative {#sec-colour-relative}
...
and to the squares of colour in the legend
...
This is an example where and encoding can be less effective
because our visual system 
does not always faithfully decode values (@sec-illusion).
...
This is another example of our visual system not producing
consistent results (@sec-illusion).
...
green, blue, indigo, violet, and back around to red.
...
hues that are both 
from the left side of the colour wheel so they appear similar,
...
decode some hues as more similar to each other
and other hues as very different from each other.
...
Similarly, in the bottom row of @fig-escape-lum, the *hue* changes slightly
as the luminance increases so that the differences are more obvious.

In effect, this is an example of a non-linear encoding (@sec-non-linear-pos)
because we take a non-linear path 
...
hue <- 0
chroma <- 40
lum <- 100*seq(.2, .8, length.out=5)
grid.newpage()
pushViewport(viewport(width=.8, layout=grid.layout(3, 1)))
pushViewport(viewport(layout.pos.row=1))
grid.rect(1:5/6, width=unit(.5, "snpc"), height=unit(.5, "snpc"),
          gp=gpar(col="black", lwd=1,
                  fill=hcl(hue, chroma, lum, 
                           fixup=FALSE)))
popViewport()
...
grid.rect(1:5/6, width=unit(.5, "snpc"), height=unit(.5, "snpc"),
          gp=gpar(col="black", lwd=1,
                  fill=hcl(hue, c(chroma, 55, 70, 55, chroma), lum, 
                           fixup=FALSE)))
popViewport()
...
grid.rect(1:5/6, width=unit(.5, "snpc"), height=unit(.5, "snpc"),
          gp=gpar(col="black", lwd=1,
                  fill=hcl(seq(hue - 50, hue, length.out=5), 
                           c(chroma, 55, 70, 55, chroma), lum, 
                           fixup=FALSE)))
popViewport()
```
...
(this is the data set that we saw in @fig-visual-model-demo).

A common approach is to encode the numeric data values using 
colour[^pseudo-color]
and it is also common to
use a rainbow colour palette for that encoding (@fig-nomads-rainbow).
...
[^rainbow-non-linear]:
    A rainbow palette is a colour encoding that is non-linear
    (@sec-non-linear-pos).
...
This is thanks to the changes in hue as well as luminance in the viridis
map; the grey-scale map on the left of @fig-nomads-viridis
varies only in terms of luminance.
