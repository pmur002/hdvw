...
We saw in @sec-mapping-visual-features that **position** is a very effective
visual feature.  It is appropriate for mapping quantitative data values
(@fig-quant-features) and it is appropriate for mapping qualitative
...
[^position-best]:
    ...
    **position**.  As we explore other sorts of mappings in later
...
not map *all* data values to position (e.g., @fig-crime and 
@fig-crime-heatmap), so there must be more to learn
about mapping data values to basic visual features.[^not-all-scatter]
In this chapter, we look more closely at the mapping
of data values to the **position** of data symbols.

[^not-all-scatter]:
    As @DBLP:journals/corr/abs-2008-11310 put it:
...
[^rel-propn]:
    Even if we have mapped a *proportion* data value to the position
...
[^surjective-over]:
    ...
    This is called a *surjective* mapping [@Ziemkiewicz2009].
...
#| echo: false
#| label: fig-ternary
#| fig-cap: A ternary plot of the data in @tbl-crime-level-tern.  We can see that for the entire decade around 60% of crimes have been of medium severity, with around 30% of low severity, and only 10% of high severity.
#| message: false
ggplot(crimeLevel3, aes(High, Low, Medium)) + 
    geom_point(aes(colour=as.factor(year))) +
    scale_colour_manual(values=grey(.8 - 1:11/14), name="") +
    coord_tern() +
    theme(tern.panel.expand=.3)
```

## Non-linear mappings {#sec-non-linear-pos}
...
Mapping data values to the position of data symbols allows
