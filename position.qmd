...
#| echo: false
#| label: fig-no-pos
#| fig-cap: This data visualisation does not make use of position. The data symbols are four concentric circles, one per ethnic group, with the radii of the circles encoding the number of offenders in each group and the colours of the circles encoding the names of the ethnic groups.  Because the circles are concentric, we can only see all of the circle for the Unknown ethnic group;  all other circles are partially obscured by circles that are drawn on top of them.
...
of data values to the **position** of data symbols and
...
## Decoding position is relative {#sec-rel-pos}
...
absolute judgements.
Position is a very important way to encode data values because
making comparisons
is one of the most important decoding tasks in
...
We can decode a value from the position of the dot, but only
relative to the ends of the dotted line;  the dot is three-quarters
of the way from the bottom to the top of the line.
We cannot decode an *absolute* value from the dot.[^rel-propn]
...
grid.circle(1/3, 3/4, unit(1.5, "mm"), gp=gpar(fill="black"))
popViewport(2)
...
grid.circle(1/3, 3/4, unit(1.5, "mm"), gp=gpar(fill="black"))
grid.circle(2/3, 1/4, unit(1.5, "mm"), gp=gpar(fill="black"))
popViewport(2)
...
In summary, we are good at *comparisons* between positions.[^comparison]
...
For example, it is easier to compare the two positions, relative to the
ends of the dotted lines, on the left
of @fig-unaligned-pos than it is to compare the two positions on the right.
...
grid.segments(.2, .25, .2, .35, gp=gpar(col="black"))
grid.segments(.7, .25, .7, .35, gp=gpar(col="black"))
grid.segments(.2, .65, .2, .75, gp=gpar(col="black"))
grid.segments(.7, .65, .7, .75, gp=gpar(col="black"))
grid.segments(.2, c(.3, .7), .7, c(.3, .7), gp=gpar(lty="dotted"))
grid.circle(c(.45, .55), c(.3, .7), r=unit(1.5, "mm"), gp=gpar(fill="black"))
popViewport(2)
pushViewport(viewport(x=.5, width=.5, just="left"))
pushViewport(viewport(x=.4, width=unit(.8, "npc")))
grid.segments(.2, .25, .2, .35, gp=gpar(col="black"))
grid.segments(.7, .25, .7, .35, gp=gpar(col="black"))
grid.segments(.3, .65, .3, .75, gp=gpar(col="black"))
grid.segments(.8, .65, .8, .75, gp=gpar(col="black"))
grid.segments(c(.2, .3), c(.3, .7), c(.7, .8), c(.3, .7), gp=gpar(lty="dotted"))
grid.circle(c(.45, .65), c(.3, .7), r=unit(1.5, "mm"), gp=gpar(fill="black"))
popViewport(2)
```

@fig-unaligned-dot shows
the number of youth
offenders per 100,000
in New Zealand for different police districts.
The data values for this map are shown in @tbl-map-data.

```{r}
#| echo: false
#| label: tbl-map-data
#| tbl-cap: The crime rate (number of offencers per 100,000 population) in 2021 for each police district of New Zealand.
kable(subset(crimeDistrict, year == 2021)[c("district", "rate")], digits=0,
      row.names=FALSE)
```

```{r}
#| echo: false
#| label: fig-unaligned-dot
#| fig-cap: A dot plot of the crime rate in each police district for 2021.
temp <- subset(crimeDistrict, year == 2021)
temp$district <- reorder(temp$district, temp$rate)
ggplot(temp) +
    geom_point(aes(rate, district), size=3) +
    geom_hline(aes(yintercept=district), linetype="dotted") +
    scale_x_continuous(limits=c(0, NA), expand=expansion(c(0, .05)))
```

In this data visualisation,
the crime rate is encoded as horizontal position.
Comparisons between different police districts is easy because
the horizontal positions all share a common baseline.

By contrast, fig-unaligned-map shows the same data
on a map.
The crime rate for each district is again encoded as horizontal position,
but it is much harder to compare crime rates between districts
because the horizontal positions are not aligned.
...
```{r}
#| echo: false
#| label: fig-unaligned-map
#| fig-height: 6
#| fig-cap: A map-based representation of the crime rate in each police district for 2021.
w <- 2
h <- 6
strip <- function(data, coords) {
    children <- list()
    for (i in 1:nrow(data)) {
        children[[i]] <- 
            grobTree(segmentsGrob(0, 0, 0, 1),
                     segmentsGrob(1, 0, 1, 1),
                     segmentsGrob(0, .5, 1, .5),
                     circleGrob(unit(h*data$height[i]/max(data$height), "mm"),
                                .5, 
                                unit(1, "mm"),
                                gp=gpar(fill="black")),
                     vp=viewport(coords$x[i], coords$y[i], 
                                 width=unit(h, "mm"), height=unit(w, "mm")))
    }
    do.call(grobTree, children)
}
stripKey <- function(data, coords) {
    vp <- viewport(.8, .2, width=unit(h, "mm"), height=unit(w, "mm"))
    grobTree(segmentsGrob(0, 0, 0, 1),
             segmentsGrob(1, 0, 1, 1),
             segmentsGrob(0, .5, 1, .5),
             textGrob("0", 0, unit(0, "mm"), just=c("left", "top")),
             textGrob("400", 1, unit(0, "mm"), just=c("left", "top")),
             textGrob("rate", 0, unit(1, "npc") + unit(2, "mm"),
                      just=c("left", "bottom")),
             vp=vp)
}
ggplot(districts) +
    geom_sf(fill="grey", colour=figbg, linewidth=.6) +
    grid_panel(strip, aes(X, Y, height=rate)) +
    grid_panel(stripKey) +
...
@fig-overplot shows a dot plot of the points scored in each match for all
...
In theory, that should allow us to decode from data points to
...
that we cannot see.
For example, we can see that there is a dot at zero points in 
@fig-overplot (for teams from the northern hemisphere), 
but we cannot see that there are actually two dots at that 
location
...
match.
We cannot decode an individual data value from each 
data symbol.[^surjective-over]
...
points that have been scored many times, but we cannot easily decode
the number of times the same set of points was scored.
We will see several other approaches to resolving overplotting
in later chapters.
...
In @fig-dot, we encoded *two* sets of data values using
**position**.
The number of offenders is encoded as the horizontal position of data
points and the ethnic group is encoded as the
vertical position of data points.
@fig-dot is effective because it encodes data values as position,
but it is also effective because it encodes data values as
*orthogonal* positions---horizontal position and vertical position.

This *cartesian coordinate system* 
is effective because 
the visual system is better at decoding horizontal and vertical
positions compared to oblique orientations.[^oblique-effect]
...
@fig-radial-pos shows an example of a data visualisation that
does not employ a cartesian coordinate system.
Like @fig-dot, it shows the number of offenders in different ethnic groups
(@tbl-crime-group-total),
and it encodes the number of offenders as position along a line.
However, the positions are not orthogonal to each other 
so it becomes more difficult to decode comparisons between
...
that sum to 1), and the data values from each variable have been 
...
decode the difference between two data values
from the difference in the positions of two data symbols,
...
#| label: fig-ternary
#| message: false
## NOT run because loading {ggtern} messes with {ggplot2} defaults
...
one quantitative variable, total number of offenders.

```{r}
#| echo: false
#| message: false
#| label: tbl-crime-type
#| tbl-cap: A table of the number of youth offenders in New Zealand in 2021 broken down by type of crime.  There are 16 different crime types in total, with only the first 6 shown here.
kable(head(crimeType2021[c("type", "count")]), digits=3, row.names=FALSE)
```

@fig-linear shows a dot plot of these data, with the total number of offenders
...
For example, we can decode that Theft is about twice as common as 
Dangerous acts.

```{r}
#| echo: false
#| label: fig-linear
#| fig-cap: A dot plot of the number of offenders in New Zealand in 2021 for different types of crime.
ct2 <- crimeType2021
ct2$type <- reorder(ct2$type, ct2$count)
ggplot(ct2) + 
    geom_segment(aes(x=-Inf, xend=Inf, y=type, yend=type), 
                 linetype="dotted") +
    geom_point(aes(x=count, y=type), size=3) +
    scale_x_continuous(name="number of offenders", 
...
data and the same encodings, but this time with a *log scale*.
Rather than encoding the number of offenders as the position
of the circles, 
the log (base 10) of the number of offenders is encoded as
...
ggplot(ct2) + 
    geom_segment(aes(x=0, xend=Inf, y=type, yend=type), 
                 linetype="dotted") +
    geom_point(aes(x=count, y=type), size=3) +
    scale_x_continuous(name="number of offenders", transform="log10", 
                       sec.axis=dup_axis(name=NULL)) +
    theme(aspect.ratio=1)
```
...
because the same horizontal distance means different amounts,
...
for Miscellaneous crimes;  we can decode that
the position of the circle data symbol
is roughly one quarter of the distance
between 10 and 100, but the corresponding
data value (18) is *not*
one quarter of the distance between 10 and 100.  It is even more difficult
...
similar to the difference between Miscellaneous crimes 
and crimes Against justice,
but the real differences between those data values are very different
(14 versus 50).
...
number of offenders for Causing injury is approximately 3, so the number
...
ggplot(ct2) + 
    geom_segment(aes(x=-Inf, xend=Inf, y=type, yend=type), 
                 linetype="dotted") +
    geom_point(aes(x=log10(count), y=type), size=3) +
    scale_x_continuous(name=expression(log[10]("number of offenders")), 
                       sec.axis=dup_axis(name=NULL)) +
    theme(aspect.ratio=1)
```
