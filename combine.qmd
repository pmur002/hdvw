...
In @fig-bar-gender, 
each data value is encoded to a **visual feature** of a data symbol
...
encodes **data summaries** to data symbols rather than raw data.
...
> One reason why a mosaic plot is effective is because it encodes proportions
...
that encodes to visual features that **interact**.
The explicit encodings involve encoding proportions as the widths and heights
...
However, the interaction of these encodings produces
...
A mosaic plot of three variables
:::
...
Almost all data visualisations involve *combinations* of encodings.
More than one set of data values are encoded as more than one
visual feature of data symbols.

The encodings involved in a bar plot---quantitative data values encoded as
lengths
...
Conversely, independence between visual features is of no help if what we
want is to decode a data summary from an emergent feature.
Furthermore, interaction between visual
features is unhelpful, or even misleading, if we cannot decode
any meaningful information from the emergent feature that results
