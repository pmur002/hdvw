...
have decreased, we cannot tell what the values are or by how much they
...
[^brath]:
    ...
    and is the source of many of the ideas in this chapter.
...
**pattern** visual feature of the white text---the letters and digits 
that make up the text.
The data values for the number of offenders in each ethnic group
are encoded as the **pattern** of the white text.
...
The pattern of a text label has a learned decoding, which means that we
can decode values from the text label without having explicitly
encoded a value as a text pattern.  This is similar to the learned decoding
of a visual object (@fig-learned-decode).
:::
...
Another example of a text-specific visual feature is the font family,
...
such as headings versus normal text, using 
the visual features of text *other than* the text pattern.
...
Font weight and font style (or slope) in theory allow for the
encoding of numerical values, but similar to area and angle, 
their relatively low accuracy means that we can only really
...
That is not to say that encoding data values as
...
## Decoding text pattern is absolute

One particular aspect of decoding text is worth emphasising again: 
we are able to decode **absolute** data values from a **text pattern**.
For example, in @fig-bar-text, we can decode the exact data value
one thousand five hundred and eighty-nine from the text pattern "1589".

This is in sharp contrast to visual features like position (@sec-rel-pos),
length (@sec-perception-relative), and colour (@sec-colour-relative),
which only allow us to decode relative values.
For example, in @fig-no-text, we can decode from the colours that
the lines represent different groups, but it is not possible to decode
the names of those groups from the colours.

This property is fundamental to the role of text in a data visualisation,
particularly axes and legends, as we will see in @sec-guides.

## Case study:  Text angle {#sec-text-angle}

@fig-text-angle shows a bar plot of the total number of offenders aged 14 to 16
in New Zealand in 2021, for different ethnic groups.
This demonstrates some very common approaches to saving space in a plot.
The title on the y-axis is rotated 90 degrees because if it had the normal
horizontal orientation, it would take up a lot of space to the left of
the plot. The tick labels on the x-axis
are rotated 45 degrees because if they had the normal horizontal orientation
they would overlap with each other.

```{r}
#| echo: false
#| label: fig-text-angle
#| fig-cap: A bar plot of the total number of offenders for different ethnic groups.
gg <- ggplot(crimeGroup2021) + 
    geom_col(aes(x=group, y=count)) +
    scale_y_continuous(expand=expansion(c(0, .05))) +
...
One problem with this common practice is that text is much more difficult
to read if it is not horizontal.[^text-angle]
In terms of @sec-configural, the **angle** of text and the **shape** of text
are visual features that have a very strong interaction---the 
decoding of text shape is most effective
when text angle is zero.
This provides a strong argument for keeping text horizontal,
which is why many of the figures in this book encode qualitative
data values as vertical position. This allows room for long group
labels to be drawn horizontally without overlapping each other.

[^text-angle]:
    @tinker1956angular was one of the first experimental studies to 
    demonstrate that reading becomes much slower and more difficult
...
**data symbols**.
In @fig-wordcloud each word is encoded as the **pattern** of 
a text data symbol and the frequency of each word is encoded as the **size**
of a text data symbol.
There is no need for a legend to decode from words to text pattern because
...
There is no legend for the encoding from word frequency to text size,
...
and this is made worse by the fact that longer words become even
...
and may attempt to decode that difference to a difference in data values
that does not exist.
...
has been sacrificed (because we have rounded some of the data values)
in order to make some of these gains.
...
@fig-tries-hist shows a histogram of the
...
Because the borders of the bars are not drawn, we get an implicit visual shape
shape from the combined bars (@sec-vis-shape-implicit).
...
@fig-tries-stem shows a stem-and-leaf 
...
As with the histogram in @fig-tries-hist, there are implicit visual shapes
with two distinct humps visible, one taller than the other.
However, in @fig-tries-stem we are *also* 
able to decode the raw data values.
...
high-level features of data summaries *and* access to raw data values.
...
are doing within a data visualisation.[^murrell-text]

[^murrell-text]:
    @Murrell_2026 provides a more detailed exploration of the role
    of text in axes and legends.
...
There are very familiar encodings of data values as circle
data symbols in @fig-legend.
...
a hemisphere from a data symbol (@sec-visual-model-groups).
...
example of encoding and decoding information as the visual
features of visual elements within a plot.
...
of a small number of absolute values with great accuracy.
...
to obtain the difference between the decoded values.
The regular sequence of "nice" values makes that mental
arithmetic much easier (@sec-tasks).
...
of indirection:  from a text label in the legend
to a coloured data symbol in the legend to a coloured data symbol in the plot.
...
(@sec-visual-focus),
which further reduces the memory load.[^direct-label-load]
...
[^florida-deaths]:
    @fig-text-danger was inspired by a 
...
labelbg <- function(data, coords) {
    ...
    lab <- textGrob("2005\nFlorida enacted\nits 'Stand Your\nGround' law",
                    ...
                    gp=gpar(fontsize=20, fontface="bold", col="white",
                            lineheight=1))
    rectGrob(grobX(lab, "west") - unit(5, "mm"),
             grobY(lab, "south"),
             grobWidth(lab) + unit(10, "mm"),
             grobHeight(lab),
             just=c("left", "bottom"),
...
[^listener-title]:
    This plot is inspired by a plot in the November 15-21 edition of 
...
the correct information from a text label.
...
@fig-corr-linear shows a *linear* plot of pairwise correlations
of seven performance measures
for teams in the 2023 Rugby World Cup (@tbl-rwc-2023).[^bullseye]
The data symbols in @fig-corr-linear are data points.
...
We can see that many pairs of 
measures have quite a strong positive correlation, 
but some pairs of measures
are negatively correlated (though often only very
weakly).
...
The different pairs of measures are encoded as the pattern of the
...
The pattern in these labels makes it easier to decode individual
measure names for each pair of measures, both because a more
visible separator has been used (a bullet point) and because
the bullet points have been aligned.
Now we can more easily decode recognisable patterns from the
...
it will describe
any data summaries like trends, clusters, outliers, and correlations,
and it will describe the main message of the visualisation, including
