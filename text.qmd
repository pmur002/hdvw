...
**TODO**
Add note in text and design that we have moved beyond what is normally considered data values and data symbols for encoding
Eg encoding structure
Eg text and guides
But nothing needs to change!
In terms of decoding 
Still just visual features and visual shakes and visual objects and congruence and learned encodings etc

**TODO**
Need to add a Decoding Text section.
Decoding text is just reading!
Decoding text is definitely a learned encoding.
Need to shift evaluation of text decoding out of @sec-features-text?
Can it wait until just before Dangers of Text?
Once we have covered labels as well?

**TODO**
Useful font selection tool?
...
**TODO**
refer back to @sec-attention (in encoding information?);  
it helps to direct attention to what
...
**TODO** 
Refer back to @sec-vis-shape-feature and @sec-vis-object-feature
...
are encoded as the **pattern** of the text---the letters and digits 
...
> It is harder to compare data values that are encoded to 
...
## Learned encodings of text {#sec-learned-text}

Link to @sec-learned.
Discuss how having learned to read allows us to decode information
from text, when the text forms known words and expressions.

## Encoding encodings {#sec-guides}

**TODO**
Footnote "guides" terminology (vs axes, legends, etc)

**TODO**
refer back to @sec-rel-pos.  We NEED the axes to decode absolute values.
ALSO, we can decode ABSOLUTE values from the text labels.
The decoding of text is NOT relative.
...
encoding from the different ages to different colours.
...
popViewport()
grid.force()
grid.edit("key::gTree::segments", grep=TRUE, global=TRUE,
          gp=gpar(lwd=2))
```

The first thing to note about the guides in @fig-crime-guides is that they
are all composed of **data symbols**, with **data values** encoded to
**visual features** of the data symbols (@sec-encoding-visual-features).
...
of the text *and* as the **shape** of the text.  Similar encodings
apply to the
x-axis tick labels.
...
**TODO**
Refer back to @fig-junk
Visual objects can encode/decode metadata just like labels
...
consider this a encoding, just one that involves encoding metadata
rather than data values (@fig-label-encoding).
...
tournament.  We established in @sec-features-text that
...
our choice of text content as we are with our selection of encodings
...
understand.
Text still has to make sense!
Just as a graphical encoding that requires effort is a waste of time,
text that requires effort is a waste of time.

**TODO**
R Journal submission 2024-88 Fig 4 y-axis labels are unreadable.
