...
map data values to the basic visual features of a data symbol.
...
[^visual-type-callback]:
    These implicit mappings are part of the reason why different visual
...
[^congruence-lit]:
    ...
    *congnitive fit*: "performance on a task will be enhanced when there is a
    ...
    but it is analogous to the idea of congruent mappings.
...
[^vertical-bars]:
    Several authors make an argument for vertical bars being preferred 
    because they correspond naturally to accumulation and growth
    [e.g., @few2004show].
...
A data visualisation will be more effective if it is visually congruent, for
example,
data symbols are larger for larger data values or 
data symbols only change if the data values change.
