...
some well-known plots, like
bar plots, are effective because they 
...
are effective because we can *accurately* decode *quantitative* data values
from those visual features.
We have also considered the *capacity* of visual features:
how many different *qualitative* data values can we decode
from a visual feature.
For example, we can only identify a small number of different
qualitative values if we encode the values using **colour**.
...
than just the accuracy of encodings or the capacity of encodings when
we encode data values as basic visual features.
...
We can explicitly encode data values as visual features so that
the decoding leads back to the same data values
that we explicitly encoded (@fig-congruent-decode)---we 
can explicitly encode data values so that
the decoding is **congruent** with the implicit decoding.[^congruence-lit]
...
[^congruence-lit]:
    ...
    accuracy and capacity and refer to 
    ...
    *congnitive fit*: "performance on a task will be enhanced when there is a
    ...
    knowledge that is imparted upon the object's user", is also
    related to the idea of congruence [@cognitive-affordance].
...
gg <- ggplot(crimeGroup2021) + 
    geom_segment(aes(x=-Inf, xend=Inf, y=group, yend=group), 
                 linetype="dotted") +
    geom_point(aes(x=count, y=group)) +
    xlab(NULL) +
    ylab(NULL) +
...
by Tier One nations from different hemispheres at Rugby World Cup matches
(@tbl-rwc-all).
...
ggplot(rwcAll) +
    geom_point(aes(x=scored, y=hemisphere), alpha=.5,
               position=position_jitter(height=.2, width=0, seed=123)) +
