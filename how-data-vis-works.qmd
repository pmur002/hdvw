...
the `total` number of
crimes for different `level`s of crime.  The data values are shown
in @tbl-crime-level-total.
...
* Which crime level has the largest total number of offenders?
* Is the total number of offenders higher for `Low`-level crimes
  of `High`-level crimes?
* Is the total number of offenders for `High`-level crimes more or less
  than half the total for `Medium-High`-level crimes?
...
* **Qualitative** data consists of categorical values that are typically
  ...
  **Nominal** data consists of just group labels,
  e.g., country names, and the only information we have is that the
...
The bar plot in @fig-bar maps the `level` of offending, which are
...
The bar plot in @fig-bar maps the `level` of offending to the
