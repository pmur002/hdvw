...
* Each `year` value is mapped to a horizontal **position** along the
  ...
  near the top edge of the plot.  The combined location provides the starting
...
* Each `year` value is mapped to a horizontal **position** along the
  ...
  near the top edge of the plot.  The combined location provides the 
  position for the white rectangle.
...
many small elements:  short lines with different **lengths**,
**angles**, and **colours**.
The angle of the lines represents wind direction and the 
length of the lines represents wind strength;  blue lines
are over the sea and green lines are over land.[^NOMADS]
...
on every individual line in the image.
...
the green region is recognised as a very familiar **object**: the
