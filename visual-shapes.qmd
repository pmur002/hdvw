...
**TODO**
refer back to @sec-pos-scarce and mention that shapes are one solution,
...
This ability to perceive data summaries, such as peaks and slopes,
...
individual data values lose their individual identity.[^surjective]

**TODO** Refer back to @sec-bar where we had bijective (but only use
that terminology in footnote), so we could decode individual data values.

[^surjective]:
    citation for surjective, injective, bijective 

    Refer back to @sec-pos-scarce
...
Refer to the aspect ratio problem:  the shape you are decoding 
is vulnerable to context!
...
Refer back to @sec-congruence (congruence).

Example from "Bars and lines: A study of graphic communication"
Zacks and Tversky (1999) (Fig 2) of inappropriate line plot:
line connecting males and females
=> some subjects made trend interpretation, the worst being
...
First point out that a boxplot is really a visual shape:
multiple data summaries generate a single "box" data symbol.
Although it is still very easy to decode individual data summaries from
the box shape.

* Case study: SinaPlots

A SinaPlot is a nice combination of decode for raw data and 
visual shape for data summary.[^sina]
...
Refer back to @sec-non-linear-pos
...
Log log scales a good example of asking what is it you want to see?
You cannot extract raw data or a compare raw data points.
But all you want is to see the data summary of correlation so no harm no foul.

Can get useful *shapes* out of non-linear scales

cite the paper (cook? hofmann?) that looked at identification of curves vs
straight lines in log-log plots?  but only in visual-shapes chapter when call
back to non-linear scales in position chapter

Nice kiwi-biased example on this thread
https://stats.stackexchange.com/questions/27951/when-are-log-scales-appropriate
But I think the point still holds that you LOSE the ability to judge size of differences - you can only decode things like order and groups (?)
