...
first, such as \textbf{position}, \textbf{angle}, and
\textbf{colour}.\footnote{Low-level visual processing involves the
  creation of ``feature maps'' that identify colours, edges, and
  orientations, along with the spatial arrangement of those basic
  features within the field of view.(Ware 2021, page=146)} This
low-level information is passed on and built upon to produce
higher-level visual elements, such as regions and \textbf{shapes}.
Information is then also integrated from prior knowledge to assist with
the identification of \textbf{objects}---things like trees, animals, and
faces.\footnote{A slightly more detailed overview is provided in Ware
  (2021) (page 20).}
...
even if the number of dots is much larger.\footnote{The ability to
  detect features without conscious effort is called ``pre-attentive''
  processing or ``pop out'' (Treisman 1985).}
...
columns on the right of Figure~\ref{fig-gestalt-group}.\footnote{These
  ...
  the same bounded region form a group by their ``proximity'',
  similarity, Todorovic (2008) provides a very accessible description of
  these principles and several others.}
...
the difference between the length of two bars?\footnote{The pioneering
  ...
  Mention Weber law and Steven's law here?}
...
to identify different values: which colour is this one?\footnote{The
  question ``Which one is this?'' is different from the question ``Are
  these two different?''. The former involves ``absolute judgements''
  (Miller 1956) to identify a stimulus from a known set of stimuli. The
  latter involves relative judgements and just-noticeable differences
  (JND), which is considerably easier and something we have a much
  higher sensitivity to (CITE please !).}

\textbf{THIS NEEDS} a revisit - check again on Munzner's expressiveness
and discriminability (see notes in DataVis/Resources)
...
greater number of colours.\footnote{``Do not use more than 10 colors for
  ...
  4th ed {[}G4.18{]} p 126. ``Choosing Effective Colours for Data
  Visualization'' Healey (1996) actually says 7 (and talks about colour
  distance, colour linearity and colour category!). Also cite the study
  that shows how capacity depends on whether points are spread out or
  arranged regularly ?}
