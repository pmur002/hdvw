...
(clusters).\footnote{In the visual perception literature, the strict
  ...
  the data visualisation literature takes a more relaxed approach
  (Szafir et al. 2016).}
...
large. However, we know that estimating averages from the positions of
multiple data symbols is quite accurate and estimating averages from the
luminance and/or chroma of multiple data symbols can also be
effective.\footnote{Gleicher et al. (2013) show that the average
  position of data points in a scatterplot can be accurately determined
  and that this is robust to a number of variations in context, such as
  the number of data points and the presence of distractors.
...
features}\label{decoding-combinations-of-visual-features}
...
combination be reduced to the decoding of the individual features. This
means that we can determine the effectiveness of a combination of
...
other cases. For example, the combinations of colour features---hue,
chroma, and luminance---all just produce different colours.
...
problem because there are many possible data summaries to consider.

However, in the specific case of decoding correlation from a scatter
plot, experiments have shown that decoding is reasonably
accurate.\footnote{Examples of studies that demonstrate accuracy for
  decoding correlation, althouhg with a tendency to underestimate, are
  Rensink and Baldridge (2010) and Cui, Kini, and Liu (2024).}
...
want to produce an emergent feature. Furthermore, interaction between
visual features is unhelpful, or even misleading, if we cannot decode
...
Refer to the aspect ratio problem: the shape you are decoding is
...
First point out that a boxplot is really a visual shape: multiple data
summaries generate a single ``box'' data symbol. Although it is still
very easy to decode individual data summaries from the box shape.
...
\textbf{TODO} Need to add a Decoding Text section. Need to shift
