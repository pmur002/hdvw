...
\section{Visual features that
interact}\label{visual-features-that-interact}
...
Decoding from a visual shape results in a data summary
(Figure~\ref{fig-data-vis-shape-decode}), for example peaks and valleys,
and, as we saw in Section~\ref{sec-summary-decode} and
Section~\ref{sec-decode-combine}, less is known about how well we can
perform this sort of decoding.

However, there is experimental support for the intuitive sense that we
can perform well at decoding qualitative features from shapes, such as
peaks and valleys, as well as decoding ordinal comparisons, such as
steeper and shallower.\footnote{Proma et al. (2025) showed that people
  are good at reproducing (therefore identifying) trends, peaks, and
  valleys, if less so at reproducing periodicity, noise/variability.
...
Section~\ref{sec-aspect-ratio} demonstrated, decodings of slopes is are
limited to physical slopes, which are dependent on the aspect ratio,
rather than decoding absolute rates of change in the data.\footnote{William
  S. Cleveland, McGill, and McGill (1988) points out that the accuracy
  of decoding slopes is only relevant to comparing the \emph{relative
  steepness} of different regions of a line. This is not accuracy in
  terms of decoding the actual rate of change in the data. For that
  purpose, we should encode the actual rate of change directly.}
...
curve shifted vertically, but we decode a decrease in the gap between
the lines.}
...
\textbf{TODO} add 3d section. Accuracy of decoding raw data values is
low, but more to decode shapes (i.e., summaries) from surfaces etc,
anyway
...
\textbf{TODO} refer back to enclosure in visual-model.Rmd
...
Figure~\ref{fig-facet-after} shows one solution: a facetted
plot.\footnote{trellis multi-panels, ggplot2 facets, GoG ??} Each
...
\textbf{TODO} Need diagram showing encoding congruent with learned
decoding (Section~\ref{sec-learned}?) and one showing encoding dissonant
with learned encoding e.g., use wrong flag for country!
(Section~\ref{sec-learned-danger})
...
flag to country (Figure~\ref{fig-data-obj-decode}).
