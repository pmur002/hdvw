...
\section{Decoding visual shapes}\label{decoding-visual-shapes}
...
names are encoded as the \textbf{pattern} of a data symbol. Each data
symbol is a pattern that allows us to distinguish between the different
countries.
...
data symbols, but they are familiar symbols. The flags convey
differences, but they also convey the more sophisticated concept of the
country names. In the terms of Section~\ref{sec-visual-processing}, the
flags are \textbf{visual objects} (Figure~\ref{fig-data-obj}).
...
(Figure~\ref{fig-learned-decode}).\footnote{``Mapping Color to Meaning
  ...
  ``Affective Congruence in Visualization Design: Influences on Reading
  Categorical Maps'' Anderson and Robinson (2021)}
...
decodings, which means that we can decode values from the visual
...
flag to country (Figure~\ref{fig-learned-decode}).
...
identify (at least some of) the country names as well
(Figure~\ref{fig-obj-decode}).
...
\subcaption{\label{fig-obj-decode}Some visual objects have learned
...
\subcaption{\label{fig-flags-direct}A scatter plot with flags as data
symbols}

}

\caption{\label{fig-data-obj-decode}Some visual objects have learned
decodings, which means that encoding data values as the visual objects,
will be effective.}
...
\textbf{TODO} Talk about how visual objects can also have basic visual
features like colour and area. Refer back to
Section~\ref{sec-vis-shape-feature}
...
simpler data symbols.

Visual objects may decode to data values or data summaries (or more
abstract/conceptual) so there are a few things to consider.

Can get visual summaries of visual objects! Whitney 2018

Decoding to data summaries, so problem of many possible decodings again
?

Think about qual ? e.g., clusters and outliers

Missing decoding a problem!

Learned decoding conflict a problem!

Congruence comes into play - big time!
...
in each police district of New Zealand.\footnote{Add citations to
  MacEachren and Bertin and Dan Carr ?} The data values for this map are
shown in Table~\ref{tbl-map-data}.
...
face (Figure~\ref{fig-data-vis-obj-stat-decode}). The mappings in this
...
\textbf{TODO} Refer back to pcp Correlation as tightly crossing lines
needs to be learned
...
\caption{\label{fig-obj-dissonant-decode}A visual feature may have more
than one learned decoding, which means that we may decode more than one
value from the visual feature.}
...
decoding, the mapping will be ineffective.}
...
unusual data symbols, but entirely novel data visualisations.\footnote{Kosslyn's
  ...
  Shah and Hoeffner (2002b) provides several examples of young students
  ...
  ``Bars and lines: A study of graphic communication'' Zacks and Tversky
  (1999) speculates on the development of graphical conventions.}

Refer back to (reading of) parallel coordinates plots.
...
\chapter{Guides}\label{sec-guides}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{index_files/mediabag/guides_files/figure-pdf/fig-crime-guides-1.pdf}
