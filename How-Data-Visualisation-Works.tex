...
We will talk about the conversion of data values to data symbols as an
\textbf{encoding} of the data values. Later chapters will explore many
different encodings. In this chapter, we will consider the reverse
...
\textbf{shapes} are identified as well as regions of different colours.
...
effort.\footnote{``the comprehension of visual displays tends to be
  ...
  for complex cognitive processes'' (Carpenter and Shah 1998, page=98)}
\end{quote}

\section{Dangers of the visual
system}\label{dangers-of-the-visual-system}
...
from a very simple arrangement of simple shapes. In other words, we are
...
These visual illusions demonstrate that the visual system can produce
incorrect results for even very simple visual tasks based on very simple
visual elements.

\begin{quote}
A data visualisation will be ineffective if it (even accidentally)
generates a visual illusion.
\end{quote}
...
In this section, we start with a very simple sort of mapping. We will
...
In terms of \textbf{decoding} ((\textbf{fid-decode?})), this means that
...
bar.\footnote{citation for surjective, injective, bijective
...
visualisation.\footnote{In this book, we use \emph{visual feature} to
  ...
  perception of data point shape Li, Wijk, and Martens (2009).} We will
explore each of these in more detail in the following sections.
...
greater number of colours.\footnote{The recommended maximum number of
  ...
  against a variety of backgrounds.'' (Ware 2021 p 126).

  Healey (1996) says the effective maximum is 7.} The difficulty only
...
dot corresponds to amongst three colours on the left and even amongst
the seven colours in the middle, but it becomes harder for the eleven
...
Main encoding is three proportions, each to oblique axis (and at least
two non-vertical and non-horizontal) =\textgreater{} retrieval of
individual proportions difficult BUT get emergent feature of position
within triangle from interaction of three position encodings
...
Example of plot that requires learning to decode
(Section~\ref{sec-learned-danger}).

Could do plot of Maori vs European vs Other proportions for different
years?

\section{Dangers of visual objects}\label{dangers-of-visual-objects}
