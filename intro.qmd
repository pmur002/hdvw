...
## Encoding and decoding {#sec-encodings}
...
Choosing which data visualisation to use can be thought of in terms
of choosing how to encode data values.
...
For example, if we want the viewer to be able to perceive a decreasing trend in
crime, it is more effective to 
encode the data values as lines (@fig-crime),
rather than a table of text (@tbl-crime),
because the viewer is able to **decode** the decreasing trend 
much more easily from the lines than from the text.
...
analyse them in terms of the way that they encode information.
...
and understanding how well those encodings can be decoded,
