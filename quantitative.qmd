...
@fig-bar-district shows a bar plot of the number of offenders
in each police district in New Zealand in 2021 (for ages 14 to 16).  
...
the same encodings as @fig-bar-district---police districts are encoded
as the vertical positions of the bars and the counts
...
the number of offenders in different ethnic groups.
...
gg <- ggplot(crimeGroup2021) +
    geom_col(aes(count, y="", fill=group), position="stack") +
...
In this data visualisation, the number of offenders is encoded to 
...
#| fig-cap: A side-by-side bar plot of the number of offenders in each ethnic group for each year from 2011 to 2021. 
ggplot(crimeGroup) +
    geom_col(aes(group, count, fill=year, group=year), position="dodge",
             colour=figbg) +
    scale_y_continuous(expand=expansion(c(0, .05)))
```
