...
extra mapping.  Country names are encoded as the **pattern** 
of the data symbols.
...
Each data symbol is a pattern that allows us to distinguish between the
different countries.
...
the flags are **visual objects** (@fig-data-obj).
...
rather than due to an implicit encoding (@fig-learned-decode).[^inferred]
...
[^inferred]:
    ...
    Influences on Reading Categorical Maps"
    Anderson and Robinson (2021)
...
(at least some of) the country names as well (@fig-obj-decode).
...
Some visual objects have learned decodings, which means that 
encoding data values as the visual objects, will be effective.
:::
...
> A plot that uses visual objects as data symbols may *not* be effective
  if the learned decodings have not been learned by all viewers.

**TODO** 
Refer back to @sec-rel-pos.
Visual shapes are one way
to decode to absolute data values (rather than just relative)

## Visual features of visual objects {#sec-vis-object-feature}

**TODO**
Talk about how visual objects can also have basic visual features
like colour and area.
Refer back to @sec-vis-shape-feature
...
then decoding proceeds as for simpler data symbols.

Visual objects may decode to data values or data summaries (or
more abstract/conceptual) so there are a few things to consider.
...
[^chart-junk]:
    ...
    + "Declutter and Focus: 
    Empirically Evaluating Design Guidelines for Effective Data Communication"
    Ajani et al (2021)
    says that "The visualization practitioner community prescribes two 
...
[^junk-pro]:
    ...
    This is the source of the "insult to man's intelligence" remark about 
    pie charts, though note the full quote:
  
    "In a sense, it might be construed as an insult to a man’s intelligence to
    show him a pie-chart, but the insult is not often resented. For if your main
    object be to get a story across, you are justified in taking that means
    which will encounter least resistance, and in making your story as simple as
    possible. For publicity purposes, the pie-chart is therefore almost
    invariably better than the bar."  [p98]
...
**TODO**
Refer back to pcp 
Correlation as tightly crossing lines needs to be learned
...
[^Arcia]:
    ...
    Give example of ambiguous colour association
    "green means profitable for financial managers but infected for health care
    workers (Brockmann, 1991)"
...
but there is no learned decoding, the mapping will be ineffective.
:::
...
[^novel-plot]:
    ...
    @Shah2002 provides several examples of young students misinterpreting
    ...
    speculates on the development of graphical conventions.
...
Refer back to @sec-ternary as example of plot type that experts in
specific fields learn to read.
