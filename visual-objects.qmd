...
to decode more complex or abstract information.
...
(@sec-differences).
The flags in @fig-flags-direct contain many different visual features---multiple
different colours and multiple different patterns---within 
the data symbol itself *and*
the flags differ from each other on many visual features at once, like
...
The encodings in a Chernoff face are *not* very accurate (@sec-accuracy;
...
In other words, we can decode data summaries from the visual objects,
for example, multivariate correlations
...
result are highly dependent on which variables are encoded as which features.
...
@fig-chernoff-colour shows a variation of the Chernoff faces in @fig-chernoff.
In this plot, the hemisphere of each country is encoded
as the **colour** of the face.
...
No data values have been encoded in the visual objects,
so no data values can be decoded (@fig-vis-clutter).
...
However, visual objects that are not related to data values
...
rather than just their heights.
...
in @fig-pic-icon-1,
plus we can decode the overall bar height, which we know 
...
flags will vary with nationality and age.
...
is fairly intuitive, ignoring the horizontal and vertical position of peaks and valleys is not intuitive.
