...
gridsvg("mappings-2.svg", width=8, height=4, res=96)
...
grid.add("bar.3-2-3-2", grep=TRUE,
         rectGrob(y=1, height=unit(1, "mm"),
                  gp=gpar(col=rgb(0,0,0,0), fill="white", lwd=.5),
                  name="rate.line"))
grid.add("bar.3-2-3-2", grep=TRUE,
...
moveAge <- atomic(dur=1, label="moveAge")
showAge <- atomic(dur=0, label="showAge")
age <- vec(moveAge, showAge, label="age")
anim <- vec(year, age, rate, durn=20, label="anim")
timings <- timing(anim)
...
ageLine <- xml_find_first(svg, '//svg:polyline[@id = "age.line.1.1"]', ns)
xml_set_attr(ageStartGroup, "style",
             "filter: drop-shadow(3px -3px 2px rgba(0,0,0,.6)")
xml_set_attr(ageLine, "style",
             "filter: drop-shadow(3px -3px 2px rgba(0,0,0,1)")
